...
-- VERSÃO CORRIGIDA - TIPOS COMPATÍVEIS
-- =====================================================
...
CREATE POLICY "Users can view their own contexts" ON user_contexts
    FOR SELECT
    USING (auth.uid()::text = user_id::text::text);
...
CORREÇÕES APLICADAS:
- Todos os auth.uid() convertidos para text para compatibilidade
- Comparações de tipos corrigidas

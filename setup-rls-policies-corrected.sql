...
-- VERSÃO CORRIGIDA - TIPOS COMPATÍVEIS COM BANCO REAL
-- =====================================================
...
1. auth.uid() retorna UUID nativo
2. Campos no banco (user_id, context_id, created_by) são TEXT/STRING
3. Solução: auth.uid()::text para converter UUID para string
...
CORREÇÕES APLICADAS:
- auth.uid()::text para converter UUID para string
- Comparações compatíveis com tipos reais do banco
- Policies testadas e funcionais

...
async function testApiMainQueryFilters() {
  console.log('🔍 TESTANDO FILTROS DA QUERY PRINCIPAL')
  console.log('=' .repeat(60))

  try {
    // 1. Simular exatamente o que a API faz na query principal
    console.log('\n1️⃣ SIMULANDO QUERY PRINCIPAL COM FILTROS...')
    ...
    // 2. Aplicar filtros de contexto para usuário matrix
    console.log('\n2️⃣ APLICANDO FILTROS DE CONTEXTO...')
    ...
    if (contextsError) {
      ...
      if (userContexts && userContexts.length > 0) {
        ...
        console.log(`🔍 Aplicando filtro de contextos: ${associatedContextIds}`)
        
        // 3. Buscar tickets com filtros de contexto
        console.log('\n3️⃣ BUSCANDO TICKETS COM FILTROS DE CONTEXTO...')
        ...
        if (recentError) {
          console.log('❌ Erro ao buscar tickets com filtros:', recentError.message)
        } else {
          console.log(`✅ Tickets com filtros: ${recentTicketsList.length} tickets`)
          recentTicketsList.forEach(ticket => {

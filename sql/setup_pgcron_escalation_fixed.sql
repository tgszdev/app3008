-- =====================================================
-- CONFIGURAÇÃO PG_CRON PARA ESCALAÇÃO AUTOMÁTICA (CORRIGIDO)
-- =====================================================
-- Este script configura execução automática de escalação
-- usando pg_cron (extensão nativa do PostgreSQL)
...
DROP FUNCTION IF EXISTS execute_auto_escalation_direct();
DROP FUNCTION IF EXISTS should_escalate_ticket(RECORD);
DROP FUNCTION IF EXISTS check_rule_conditions(RECORD, RECORD);
DROP FUNCTION IF EXISTS execute_ticket_escalation(RECORD);
DROP FUNCTION IF EXISTS execute_rule_actions(RECORD, RECORD, UUID);
DROP FUNCTION IF EXISTS process_escalation_emails_direct();

-- 4. Função para executar escalação automática
...
-- 5. Função para executar escalação de um ticket específico
...
-- 6. Função para verificar se escalação deve ser executada
...
-- 7. Função para executar ações de escalação
...
-- 8. Função para processar e-mails de escalação
...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            -- Simular envio de e-mail (em produção, integrar com serviço real)
...
-- 9. Função para chamar API de escalação automática
...
-- 10. Função para chamar API de processamento de e-mails
...
-- 3. Verifique se os jobs foram criados com SELECT * FROM cron.job;
-- 4. Monitore as execuções com SELECT * FROM cron.job_run_details;
-- =====================================================
